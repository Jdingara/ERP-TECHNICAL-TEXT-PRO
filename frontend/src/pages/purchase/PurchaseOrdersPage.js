...
export default function PurchaseOrdersPage() {
    ...
    const [detail,   setDetail]   = useState(null);   // PO detail view
    const [newPO,    setNewPO]    = useState(false);  // new PO form
    ...
    return (
        <div style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <button onClick={() => { setMsg(''); setNewPO(true); }} style={btn('#10b981')}>+ New Purchase Order</button>
            </div>
            ...
            {detail && (
                <div style={overlay} onClick={() => setDetail(null)}>
                    <div style={{ ...mBox, width: 720 }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <h3 style={{ margin: 0 }}>{detail.po_number}</h3>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
                            ...
                            <InfoRow label="Order Date" value={detail.order_date} />
                            <InfoRow label="Expected" value={detail.expected_date || '—'} />
                            <InfoRow label="Total Amount" value={`₹${detail.total_amount}`} />
                        </div>
                        <b style={{ fontSize: 13 }}>Line Items:</b>
                        <table style={{ ...tableS, marginTop: 8 }}><thead><tr style={{ background: '#334155', color: '#fff' }}>
            ...
            {newPO && (
                <div style={overlay}>
                    <div style={{ ...mBox, width: 820 }}>
                        <h3 style={{ margin: '0 0 20px' }}>New Purchase Order</h3>
...
const InfoRow = ({ label, value }) => (
    <div><div style={{ fontSize: 11, color: '#64748b', marginBottom: 2 }}>{label}</div><div style={{ fontWeight: 600, fontSize: 13 }}>{value}</div></div>
);

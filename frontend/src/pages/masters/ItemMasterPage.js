            ...
            <table style={tableS}><thead><tr style={{ background:'#1e293b', color:'#fff' }}>
                ...
                {rows.map((r,i)=>(
                    <tr key={r.id} style={{ background: i%2===0?'#f8fafc':'#fff' }}>
                        <td style={tdS}><b>{r.item_code}</b></td>
                ...
                {rows.length===0 && <tr><td colSpan={6} style={{ textAlign:'center',padding:40,color:'#94a3b8' }}>No items</td></tr>}
            </tbody></table>
            {modal && (
                <div style={overlay}><div style={mBox}>
                    <h3 style={{ margin:'0 0 20px' }}>{editId?'Edit Item':'Add Item'}</h3>
                    <div style={grid2}>
                        <F label="Item Code *"><input style={inpS} {...inp('item_code')} /></F>
                        <F label="Item Name *"><input style={inpS} {...inp('item_name')} /></F>
                        <F label="Type">
                            <select style={inpS} {...inp('item_type')}>
                                {TYPES.map(t=><option key={t} value={t}>{t}</option>)}
                            </select>
                        </F>
                        <F label="UOM">

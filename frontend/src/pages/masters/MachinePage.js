...
export default function MachinePage() {
    ...
    const openEdit = (r) => { setForm({ machine_code:r.machine_code, machine_name:r.machine_name,
        machine_type:r.machine_type, location_id:r.location_id||'', capacity_per_day:r.capacity_per_day, uom_id:r.uom_id||'' });
    ...
    const del = async (id) => { if(!window.confirm('Deactivate?')) return; await fetch(`/api/masters/machines/${id}/`, {method:'DELETE',credentials:'include'}); load(); };
                ...
                <table style={tableS}><thead><tr style={{ background:'#1e293b', color:'#fff' }}>
                    ...
                    {rows.map((r,i)=>(
                        <tr key={r.id} style={{ background: i%2===0?'#f8fafc':'#fff' }}>
                            <td style={tdS}><b>{r.machine_code}</b></td>
                    ...
                    {rows.length===0 && <tr><td colSpan={7} style={{ textAlign:'center',padding:40,color:'#94a3b8' }}>No machines</td></tr>}
                </tbody></table>
            </div>
            {modal && (
                <div style={overlay}><div style={mBox}>
                    <h3 style={{ margin:'0 0 20px' }}>{editId?'Edit Machine':'Add Machine'}</h3>
                    <div style={grid2}>
                        ...
                        <F label="Machine Type">
                            <select style={inpS} {...inp('machine_type')}>
                                {TYPES.map(t=><option key={t} value={t}>{t}</option>)}
                            </select>
                        </F>
                        <F label="Location">

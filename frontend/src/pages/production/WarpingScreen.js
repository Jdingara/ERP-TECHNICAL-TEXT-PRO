...
function WarpingScreen() {
    ...
    const inp  = {
        padding: '8px 10px', borderRadius: 6, border: `1px solid ${pt.colors.border}`,
        backgroundColor: pt.colors.inner, color: pt.colors.text, fontSize: 13, width: '100%', boxSizing: 'border-box',
    };
        ...
        <div style={{ padding: '24px 28px', fontFamily: 'Inter, sans-serif', color: pt.colors.text, minHeight: '100vh', backgroundColor: pt.colors.outer }}>
            ...
            {showForm && (
                <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                    <div style={{ backgroundColor: pt.colors.card, borderRadius: 12, width: '100%', maxWidth: 780, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                            ...
                            {/* Machine */}
                            <div>
                                <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Warping Machine *</label>
                        ...
                        <div style={{ margin: '20px 0 10px', padding: '14px 16px', backgroundColor: pt.colors.inner, borderRadius: 8, border: '1px solid #1e3a5f' }}>
                            <p style={{ margin: '0 0 12px', color: '#3b82f6', fontWeight: 600, fontSize: 13 }}>Warping Parameters</p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Beam Count</label>
                                    <input type="number" value={form.beam_count} onChange={e => setForm(f => ({ ...f, beam_count: e.target.value }))} style={inp} placeholder="1" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Total Ends (threads)</label>
                                    <input type="number" value={form.total_ends} onChange={e => setForm(f => ({ ...f, total_ends: e.target.value }))} style={inp} placeholder="e.g. 3600" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Reed Count</label>
                                    <input value={form.reed_count} onChange={e => setForm(f => ({ ...f, reed_count: e.target.value }))} style={inp} placeholder="e.g. 60/10cm" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Warp Length (m)</label>
                                ...
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Warp Tension (g)</label>
                                ...
                                <div>
                                    <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Beam Numbers</label>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                            <div>
                                <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Output Qty (kg/m)</label>
                        ...
                        {/* Lot Inputs */}
                        <div style={{ margin: '16px 0', padding: '14px 16px', backgroundColor: pt.colors.inner, borderRadius: 8 }}>
            ...
            <div style={{ backgroundColor: pt.colors.card, borderRadius: 12, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {entries.length === 0 && (
                            <tr><td colSpan={10} style={{ ...cell, textAlign: 'center', color: pt.colors.muted, padding: 40 }}>No warping entries yet. Create one above.</td></tr>
                        )}
                        {entries.map(e => (

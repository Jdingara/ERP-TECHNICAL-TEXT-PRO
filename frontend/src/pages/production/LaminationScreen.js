// ============================================================
// FILE: pages/production/LaminationScreen.js
// PURPOSE: Lamination stage — film type, coat weight, curing temp
// ============================================================
...
const FILM_TYPES = ['PVC', 'TPU', 'PE', 'PP', 'Nylon', 'EVA', 'Other'];

function LaminationScreen() {
    ...
    const blankForm = {
        ...
        film_type: 'PVC', film_gsm: '', coat_weight_gsm: '',
        adhesive_type: '', line_speed_mpm: '', curing_temp: '',
        lot_inputs: [],
    };
    ...
    const load = useCallback(() => {
        fetch(`${API}/production/stages/lamination/`, { credentials: 'include' })
    ...
    useEffect(() => {
        ...
        fetch(`${API}/masters/processes/`, { credentials: 'include' })
            .then(r => r.json()).then(d => setProcesses((d.processes || []).filter(p =>
                p.process_name?.toLowerCase().includes('laminat') ||
                p.process_name?.toLowerCase().includes('coat'))));
    ...
    const submit = async () => {
        ...
        setLoading(true);
        try {
            const r = await fetch(`${API}/production/stages/lamination/`, {
                method: 'POST', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...form,
                    film_gsm: parseFloat(form.film_gsm) || 0,
                    coat_weight_gsm: parseFloat(form.coat_weight_gsm) || 0,
                    line_speed_mpm: parseFloat(form.line_speed_mpm) || 0,
                    curing_temp: parseInt(form.curing_temp) || 0,
                    output_quantity: parseFloat(form.output_quantity) || 0,
                    rejection_qty: parseFloat(form.rejection_qty) || 0,
                }),
            });
            const d = await r.json();
            if (d.success) { flash('Lamination entry saved.'); setShowForm(false); setForm(blankForm); load(); }
    ...
    return (
        <div style={{ padding: '24px 28px', fontFamily: 'Inter, sans-serif', color: '#f1f5f9', minHeight: '100vh', backgroundColor: '#0b1120' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <div>
                    <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Lamination Screen</h2>
                    <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 13 }}>Film type · GSM · Coat weight · Curing temperature</p>
                </div>
                <button onClick={() => { setShowForm(true); setForm(blankForm); }}
                    style={{ padding: '9px 20px', borderRadius: 8, border: 'none', backgroundColor: '#ec4899', color: '#fff', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>
                    + New Lamination Entry
                </button>
            </div>
            ...
            {showForm && (
                <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                    <div style={{ backgroundColor: '#1e293b', borderRadius: 12, width: '100%', maxWidth: 760, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <h3 style={{ margin: 0, fontSize: 18 }}>New Lamination Entry</h3>
                        ...
                        {/* Lamination Params */}
                        <div style={{ margin: '0 0 14px', padding: '16px', backgroundColor: '#0f172a', borderRadius: 8, border: '1px solid #831843' }}>
                            <p style={{ margin: '0 0 14px', color: '#ec4899', fontWeight: 700, fontSize: 13 }}>Lamination Parameters</p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Film Type</label>
                                    <select value={form.film_type} onChange={e => setForm(f => ({ ...f, film_type: e.target.value }))} style={inp}>
                                        {FILM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Film GSM</label>
                                    <input type="number" step="0.1" value={form.film_gsm} onChange={e => setForm(f => ({ ...f, film_gsm: e.target.value }))} style={inp} placeholder="e.g. 150" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Coat Weight (GSM)</label>
                                    <input type="number" step="0.1" value={form.coat_weight_gsm} onChange={e => setForm(f => ({ ...f, coat_weight_gsm: e.target.value }))} style={inp} placeholder="e.g. 200" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Adhesive Type</label>
                                    <input value={form.adhesive_type} onChange={e => setForm(f => ({ ...f, adhesive_type: e.target.value }))} style={inp} placeholder="e.g. Polyurethane" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Line Speed (m/min)</label>
                                    <input type="number" step="0.1" value={form.line_speed_mpm} onChange={e => setForm(f => ({ ...f, line_speed_mpm: e.target.value }))} style={inp} placeholder="e.g. 8.5" />
                                </div>
                                <div>
                                    <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 5 }}>Curing Temp (°C)</label>
                                    <input type="number" value={form.curing_temp} onChange={e => setForm(f => ({ ...f, curing_temp: e.target.value }))} style={inp} placeholder="e.g. 160" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
                            ...
                            <button onClick={submit} disabled={loading} style={{ padding: '9px 24px', borderRadius: 8, border: 'none', backgroundColor: '#ec4899', color: '#fff', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>
                                {loading ? 'Saving…' : 'Save Lamination Entry'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div style={{ backgroundColor: '#1e293b', borderRadius: 12, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr>
                            {['Entry #', 'Date', 'Prod. Order', 'Machine', 'Film Type', 'Film GSM', 'Coat wt', 'Speed m/min', 'Curing °C', 'Output m', 'Status', ''].map(h => (
                    ...
                    <tbody>
                        {entries.length === 0 && (
                            <tr><td colSpan={12} style={{ ...cell, textAlign: 'center', color: '#475569', padding: 40 }}>No lamination entries yet.</td></tr>
                        )}
                        {entries.map(e => (
                            <tr key={e.id}>
                                <td style={{ ...cell, color: '#f472b6', fontWeight: 600 }}>{e.entry_number}</td>
                                <td style={cell}>{e.entry_date}</td>
                                <td style={cell}>{e.prod_order_number}</td>
                                <td style={cell}>{e.machine_code}</td>
                                <td style={cell}>
                                    <span style={{ backgroundColor: '#be185d22', color: '#f472b6', padding: '2px 8px', borderRadius: 4, fontSize: 12 }}>
                                        {e.lamination?.film_type || '—'}
                                    </span>
                                </td>
                                <td style={{ ...cell, textAlign: 'center' }}>{e.lamination?.film_gsm ?? '—'}</td>
                                <td style={{ ...cell, textAlign: 'center' }}>{e.lamination?.coat_weight_gsm ?? '—'}</td>
                                <td style={{ ...cell, textAlign: 'center' }}>{e.lamination?.line_speed_mpm ?? '—'}</td>
                                <td style={{ ...cell, textAlign: 'center', color: '#f97316' }}>{e.lamination?.curing_temp ?? '—'}</td>
...
export default LaminationScreen;

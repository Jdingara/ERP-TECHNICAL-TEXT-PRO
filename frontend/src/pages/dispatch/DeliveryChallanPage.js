...
function PrintableChallан({ dc, dispatches, onClose }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ backgroundColor: pt.colors.card, color: '#000', borderRadius: 8, width: '100%', maxWidth: 760, maxHeight: '90vh', overflowY: 'auto', padding: 32 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, borderBottom: '2px solid #000', paddingBottom: 16 }}>
                    <div>
                        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, color: pt.colors.text }}>MEI TEXZ</h1>
                        <p style={{ margin: '4px 0 0', fontSize: 12, color: pt.colors.muted }}>Technical Textile Manufacturer</p>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                        <h2 style={{ margin: 0, fontSize: 20, color: pt.colors.text, fontWeight: 700 }}>DELIVERY CHALLAN</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 20 }}>
                    <div style={{ padding: 12, border: `1px solid ${pt.colors.border}`, borderRadius: 6 }}>
                        <p style={{ margin: '0 0 4px', fontSize: 11, fontWeight: 700, color: pt.colors.dimText, textTransform: 'uppercase' }}>Consignee</p>
                        <p style={{ margin: 0, fontWeight: 700, fontSize: 14 }}>{dc.customer_name}</p>
                        {dc.delivery_address && <p style={{ margin: '4px 0 0', fontSize: 12, color: pt.colors.muted, whiteSpace: 'pre-line' }}>{dc.delivery_address}</p>}
                    </div>
                    <div style={{ padding: 12, border: `1px solid ${pt.colors.border}`, borderRadius: 6 }}>
                        <p style={{ margin: '0 0 8px', fontSize: 11, fontWeight: 700, color: pt.colors.dimText, textTransform: 'uppercase' }}>Transport Details</p>
                        {[['Vehicle No', dc.vehicle_number], ['Driver', dc.driver_name], ['LR Number', dc.lr_number], ['Transporter', dc.transporter], ['E-Way Bill', dc.eway_bill]].map(([k, v]) => v ? (
                            <div key={k} style={{ display: 'flex', gap: 8, marginBottom: 3 }}>
                                <span style={{ fontSize: 12, color: pt.colors.dimText, minWidth: 80 }}>{k}:</span>
                ...
                {dispatch && (
                    <div style={{ marginBottom: 16, padding: '8px 12px', backgroundColor: pt.colors.outer, borderRadius: 6, fontSize: 12 }}>
                        <span style={{ color: pt.colors.dimText }}>Dispatch Ref: </span>
                        <span style={{ fontWeight: 600 }}>{dispatch.dispatch_number}</span>
                        <span style={{ color: pt.colors.dimText, marginLeft: 16 }}>SO Ref: </span>
                    ...
                    <tbody>
                        {dispatch?.lines?.length > 0
                            ? dispatch.lines.map((line, i) => (
                                <tr key={i} style={{ borderBottom: `1px solid ${pt.colors.border}` }}>
                            ...
                            : (
                                <tr>
                                    <td colSpan={6} style={{ padding: '16px 12px', fontSize: 12, color: pt.colors.muted, textAlign: 'center' }}>
                ...
                {dc.remarks && (
                    <div style={{ marginBottom: 24, padding: '10px 12px', backgroundColor: pt.colors.outer, borderRadius: 6 }}>
                    ...
                    {['Prepared By', 'Authorised By', 'Received By (Customer Stamp)'].map(label => (
                        <div key={label} style={{ textAlign: 'center' }}>
                            <div style={{ borderTop: '1px solid #000', paddingTop: 6 }}>
                                <p style={{ margin: 0, fontSize: 11, color: pt.colors.dimText }}>{label}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 32 }}>
                    <button onClick={onClose} style={{ padding: '8px 20px', borderRadius: 6, border: '1px solid #334155', backgroundColor: 'transparent', color: pt.colors.text, cursor: 'pointer', fontSize: 13 }}>Close</button>
...
function DeliveryChallanPage() {
    ...
    const th   = { ...cell, backgroundColor: '#0f172a', color: pt.colors.muted, fontWeight: 600, textTransform: 'uppercase', fontSize: 11 };
    const inp  = { padding: '8px 10px', borderRadius: 6, border: '1px solid #334155', backgroundColor: '#0f172a', color: pt.colors.text, fontSize: 13, width: '100%', boxSizing: 'border-box' };

    return (
        <div style={{ padding: '24px 28px', fontFamily: 'Inter, sans-serif', color: pt.colors.text, minHeight: '100vh', backgroundColor: '#0b1120' }}>
            {printDC && (
            ...
            {!showForm ? (
                <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                        <div>
                            ...
                            <p style={{ margin: '4px 0 0', color: pt.colors.dimText, fontSize: 13 }}>Create DC · Assign vehicle · Print for driver</p>
                        </div>
                    ...
                    <div style={{ backgroundColor: '#1e293b', borderRadius: 12, overflow: 'hidden' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                                {challans.length === 0 && (
                                    <tr><td colSpan={8} style={{ ...cell, textAlign: 'center', color: pt.colors.muted, padding: 40 }}>No delivery challans yet. Create one above.</td></tr>
                                )}
                                {challans.map(dc => (
                                    <tr key={dc.id}>
                ...
                <div style={{ backgroundColor: '#1e293b', borderRadius: 12, padding: 28, maxWidth: 800 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid #334155' }}>
                        <button onClick={() => setShowForm(false)}
                            style={{ padding: '7px 16px', background: '#0f172a', color: pt.colors.muted, border: '1px solid #334155', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                            ← Back to Challans
                        </button>
                        <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>New Delivery Challan</h3>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                        <div style={{ gridColumn: '1/-1' }}>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Dispatch Entry (link to dispatch)</label>
                        ...
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>DC Date</label>
                            <input type="date" value={form.dc_date} onChange={e => setForm(f => ({ ...f, dc_date: e.target.value }))} style={inp} />
                        </div>
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Vehicle Number</label>
                        ...
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Driver Name</label>
                            <input value={form.driver_name} onChange={e => setForm(f => ({ ...f, driver_name: e.target.value }))} style={inp} placeholder="Driver name" />
                        </div>
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>LR Number</label>
                            <input value={form.lr_number} onChange={e => setForm(f => ({ ...f, lr_number: e.target.value }))} style={inp} placeholder="Lorry receipt number" />
                        </div>
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Transporter</label>
                        ...
                        <div>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>E-Way Bill No</label>
                            <input value={form.eway_bill} onChange={e => setForm(f => ({ ...f, eway_bill: e.target.value }))} style={inp} placeholder="12-digit e-way bill" />
                        </div>
                        <div style={{ gridColumn: '1/-1' }}>
                            <label style={{ fontSize: 12, color: pt.colors.muted, display: 'block', marginBottom: 5 }}>Delivery Address</label>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 24 }}>
                        <button onClick={() => setShowForm(false)} style={{ padding: '9px 20px', borderRadius: 8, border: '1px solid #334155', backgroundColor: 'transparent', color: pt.colors.muted, cursor: 'pointer', fontSize: 13 }}>Cancel</button>

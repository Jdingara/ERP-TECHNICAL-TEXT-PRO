...
function ForecastForm({ customers, products, onSave, onClose, existing }) {
    ...
    const inp = { padding: '7px 10px', border: `1px solid ${pt.colors.border}`, borderRadius: 7, fontSize: 13, width: '100%', boxSizing: 'border-box', outline: 'none' };
    const lbl = { display: 'block', fontSize: 12, fontWeight: 600, color: pt.colors.muted, marginBottom: 4 };

    return (
        <div style={{ background: pt.colors.card, borderRadius: 12, padding: 28, maxWidth: 1000 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, paddingBottom: 16, borderBottom: `1px solid ${pt.colors.border}` }}>
                <button onClick={onClose} style={{ padding: '7px 16px', background: '#f1f5f9', color: pt.colors.text, border: `1px solid ${pt.colors.border}`, borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                    ← Back to Forecasts
                </button>
                <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>{existing ? 'Edit Forecast' : 'New Customer Forecast'}</h3>
            </div>
            ...
            <div style={{ background: pt.colors.inner, borderRadius: 10, padding: 16, marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: pt.colors.text }}>Product Lines</div>
                ...
                {lines.length === 0 && (
                    <div style={{ textAlign: 'center', color: pt.colors.muted, fontSize: 13, padding: '16px 0' }}>No products added yet. Click + Add Product.</div>
                )}
                ...
                {lines.length > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 80px 32px', gap: 8, marginTop: 4 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: pt.colors.muted }}>Product</div>
                        <div style={{ fontSize: 11, color: pt.colors.dimText }}>{form.month_1_label || 'M1 (kg)'}</div>
                        <div style={{ fontSize: 11, color: pt.colors.dimText }}>{form.month_2_label || 'M2 (kg)'}</div>
                        <div style={{ fontSize: 11, color: pt.colors.dimText }}>{form.month_3_label || 'M3 (kg)'}</div>
                        <div style={{ fontSize: 11, color: pt.colors.dimText, textAlign: 'center' }}>Route</div>
                        <div />
                    </div>
                )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={onClose} style={{ padding: '8px 18px', background: 'transparent', color: pt.colors.dimText, border: `1px solid ${pt.colors.border}`, borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>Cancel</button>
                ...
                <>
                    <div style={{ background: 'linear-gradient(135deg,#1e3a5f,#7c3aed)', borderRadius: 12, padding: '20px 24px', marginBottom: 24, color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button onClick={() => { setEditing(null); setShowForm(true); }}
                            style={{ padding: '9px 20px', background: pt.colors.card, color: '#7c3aed', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: 13 }}>
                            + New Forecast
                        </button>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                        <input placeholder="Search by title or customer…" value={search} onChange={e => setSearch(e.target.value)}
                            style={{ padding: '8px 12px', border: `1px solid ${pt.colors.border}`, borderRadius: 8, fontSize: 13, width: 280, outline: 'none' }} />
                        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                            style={{ padding: '8px 12px', border: `1px solid ${pt.colors.border}`, borderRadius: 8, fontSize: 13, outline: 'none' }}>
                    ...
                    <div style={{ background: pt.colors.card, border: `1px solid ${pt.colors.border}`, borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 4px #0001' }}>
                        {loading ? (
                            <div style={{ textAlign: 'center', padding: 48, color: pt.colors.muted }}>Loading…</div>
                        ) : (
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                                <thead>
                                    <tr style={{ background: pt.colors.inner, color: '#fff' }}>
                                ...
                                <tbody>
                                    {filtered.length === 0 && (
                                        <tr><td colSpan={8} style={{ textAlign: 'center', padding: 48, color: pt.colors.muted }}>No forecasts found. Click + New Forecast to create one.</td></tr>
                                    )}
                                    {filtered.map((fc, i) => (
                                        <tr key={fc.id} style={{ background: i % 2 === 0 ? '#fff' : '#f8fafc', borderBottom: `1px solid ${pt.colors.border}` }}>
                                            <td style={{ padding: '10px 14px' }}><b style={{ color: pt.colors.text }}>{fc.title}</b></td>
                                            <td style={{ padding: '10px 14px', color: pt.colors.muted }}>{fc.customer_code} — {fc.customer_name}</td>
                                            <td style={{ padding: '10px 14px', color: pt.colors.dimText, fontSize: 12 }}>{fc.period_from} → {fc.period_to}</td>
                                            <td style={{ padding: '10px 14px', color: pt.colors.dimText, fontSize: 12 }}>{fc.month_1_label}</td>
                                            <td style={{ padding: '10px 14px', color: pt.colors.dimText, fontSize: 12 }}>{fc.month_2_label}</td>
                                            <td style={{ padding: '10px 14px', color: pt.colors.dimText, fontSize: 12 }}>{fc.month_3_label}</td>
